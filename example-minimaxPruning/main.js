...
const minimax = (root, depth, isMaxTurn, alpha, beta) => {
  if (depth == 0) return root.value;

  if (isMaxTurn) {
    let maxValue = -Infinity;
    for (let i = 0; i < root.children.length; i++) {
      let value = minimax(root.children[i], depth - 1, !isMaxTurn, alpha, beta);
      maxValue = Math.max(value, maxValue);
      alpha = Math.max(value, alpha);
      if (alpha >= beta) {
        console.log("CUT ", root.children[i]);
        break;
      }
    }
    return maxValue;
...
console.log(minimax(N1, 3, true, -Infinity, Infinity));
